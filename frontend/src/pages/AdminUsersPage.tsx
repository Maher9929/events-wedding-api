...
const AdminUsersPage = () => {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      toastService.error(
        t('common.admin.users_load_error', 'Failed to load users'),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleRoleChange = async (userId: string, newRole: string) => {
    try {
      ...
      toastService.success(
        t('common.admin.role_update_success', 'Role updated successfully'),
      );
    } catch (_error) {
      toastService.error(
        t('common.admin.role_update_error', 'Failed to update role'),
      );
    }
  };
  ...
  const handleDeleteUser = async (userId: string) => {
    const ok = await confirm({
      ...
      message: t(
        'common.admin.confirm_delete_user_msg',
    ...
    try {
      ...
      toastService.success(
        t('common.admin.user_delete_success', 'User deleted successfully'),
      );
    } catch (_error) {
      toastService.error(
        t('common.admin.user_delete_error', 'Failed to delete user'),
      );
    } finally {
      setDeletingId(null);
    }
  ...
  const statCards = [
    {
      label: t('common.admin.total_users', 'Total users'),
      value: stats.total,
      icon: 'fa-users',
      color: 'bg-blue-50 text-blue-600',
    },
    {
      label: t('common.roles.client_plural', 'Clients'),
      value: stats.clients,
      icon: 'fa-user',
      color: 'bg-indigo-50 text-indigo-600',
    },
    {
      label: t('common.roles.provider_plural', 'Providers'),
      value: stats.providers,
      icon: 'fa-store',
      color: 'bg-green-50 text-green-600',
    },
    {
      label: t('common.roles.admin_plural', 'Admins'),
      value: stats.admins,
      icon: 'fa-shield',
    ...
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">
          {t('common.admin.users', 'User Management')}
        </h1>
        <p className="text-gray-500">
          {t(
            'common.admin.users_subtitle',
      ...
      <div className="flex flex-col gap-3 md:flex-row">
        ...
        >
          <option value="all">
            {t('common.admin.all_roles', 'All roles')}
          </option>
            ...
            <thead className="border-b border-gray-100 bg-gray-50 text-sm font-bold text-gray-700">
              <tr>
                ...
                <th className="px-6 py-4">
                  {t('common.admin.registered_at', 'Registered')}
                </th>
                        ...
                        <div className="flex items-center gap-3">
                          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary">
                            {user.avatar_url ? (
                              ...
                              <i
                                className={`fa-solid ${role.icon} text-sm`}
                              ></i>
                            )}
                          </div>
                          <span className="text-sm font-bold text-gray-900">
                            {user.full_name || t('common.not_available', 'N/A')}
                          </span>
                        </div>
                      </td>
