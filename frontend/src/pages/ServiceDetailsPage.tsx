...
const ServiceDetailsPage = () => {
  ...
  const loadUnavailableDates = useCallback(
    async (providerId: string) => {
      try {
        ...
        const res = await bookingsService.getUnavailableDates(
          providerId,
          start,
          end,
        );
  ...
  useEffect(() => {
    if (service) {
      document.title = `${service.title} | ${t('service.listing.title', 'الخدمات')} | Doha Events`;
    } else {
      document.title = `${t('service.listing.title', 'الخدمات')} | Doha Events`;
    }
  }, [service, t]);
  ...
  useEffect(() => {
    ...
    void servicesService
      ...
      .then((data) => {
        const svc =
          (data as { data?: ExtendedServiceItem }).data ||
          (data as ExtendedServiceItem);
        setService(svc);
        if (svc?.provider_id) void loadUnavailableDates(svc.provider_id);
      })
      .catch(() =>
        toastService.error(
          t('service.errors.load_failed', 'فشل تحميل تفاصيل الخدمة'),
        ),
      )
      .finally(() => setLoading(false));
    loadReviews();
  }, [id, t, loadUnavailableDates, loadReviews]);
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      toastService.info(
        t('common.favorite_unavailable', 'المفضلة غير متاحة حالياً'),
      );
    } catch (err) {
      toastService.error(
        err instanceof Error
          ? err.message
          : t('service.errors.favorite_failed', 'فشل تحديث المفضلة'),
      );
    } finally {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      void navigate(`${basePath}/messages`);
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      toastService.error(
        t('service.errors.review_failed', 'فشل إرسال التقييم'),
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleCreateBooking = async (includeAmountParam = false) => {
    if (!isAuthenticated) {
      void navigate('/auth/login');
      return;
    }
    if (!bookingDate) {
      toastService.error(
        t(
          includeAmountParam
            ? 'bookings.errors.select_date_above'
            : 'bookings.errors.select_date',
          includeAmountParam
            ? 'يرجى اختيار تاريخ الفعالية أعلاه'
            : 'يرجى اختيار تاريخ الفعالية',
        ),
      );
      if (includeAmountParam) window.scrollTo({ top: 0, behavior: 'smooth' });
      return;
    }
    if (unavailableDates.includes(bookingDate)) {
      toastService.error(
        t('bookings.date_unavailable', 'هذا التاريخ غير متاح'),
      );
      return;
    }
    if (!service) return;
    ...
    try {
      ...
      const bookingData = (booking as { data?: Booking }).data || booking;
      toastService.success(
        t('bookings.create_success', 'تم إنشاء الحجز بنجاح'),
      );
      if (bookingData?.id) {
        const amountQuery = includeAmountParam
          ? `?amount=${service.base_price}`
          : '';
        void navigate(`/client/payment/${bookingData.id}${amountQuery}`);
      }
    } catch {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-bglight font-tajawal">
        <p className="text-gray-500 mb-4">
          {t('service.listing.no_services', 'لا توجد خدمة')}
        </p>
        <button
          onClick={() => void navigate('/services')}
          className="px-4 py-2 bg-primary text-white rounded-xl"
        >
          {t('common.back_to_services', 'العودة للخدمات')}
        </button>
      </div>
    );
  }
  ...
  const images = service.images?.length
    ? service.images
    : [
        'https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=400&q=60&fm=webp',
      ];
  const avgRating =
    reviews.length > 0
  ...
  return (
    ...
    >
      {/* Header */}
      <header className="bg-white sticky top-0 z-50 shadow-sm lg:hidden">
        <div className="px-5 py-4 flex items-center justify-between">
          ...
          <h1 className="text-lg font-bold text-gray-900">
            {t('service.listing.title', 'الخدمات')}
          </h1>
          <button
          ...
          >
            <i
              className={`${isFavorite ? 'fa-solid' : 'fa-regular'} fa-heart text-red-500 ${togglingFav ? 'opacity-50' : ''}`}
            ></i>
          </button>
        </div>
      </header>
      ...
      <main>
        <div className="hidden px-5 pb-4 pt-2 lg:block">
          <div className="flex items-center justify-between">
            <button
              onClick={() => void navigate('/services')}
              className="inline-flex h-11 items-center gap-2 rounded-2xl border border-gray-200 bg-white px-4 text-sm font-bold text-gray-700 shadow-sm transition hover:border-primary/30 hover:text-primary"
            >
              <i
                className={`fa-solid ${i18n.language === 'ar' ? 'fa-arrow-right' : 'fa-arrow-left'}`}
              ></i>
              {t('common.back_to_services', 'العودة للخدمات')}
            </button>
            <button
              onClick={() => void handleToggleFavorite()}
              disabled={togglingFav}
              className="inline-flex h-11 items-center gap-2 rounded-2xl border border-gray-200 bg-white px-4 text-sm font-bold text-gray-700 shadow-sm transition hover:border-red-200 hover:text-red-500 disabled:opacity-50"
            >
              <i
          ...
          {service.is_featured && (
            <div className="absolute top-4 right-4 px-3 py-1.5 rounded-full bg-accent shadow-sm">
              <span className="text-xs font-bold text-gray-900">
                <i className="fa-solid fa-crown mx-1"></i>
                {t('service.listing.featured_title', 'خدمة مميزة')}
              </span>
            </div>
          )}
        </section>
          ...
          <div className="flex items-center gap-4 mb-4">
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm text-gray-500">
                ({reviews.length} {t('service.details.reviews', 'التقييمات')})
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-bglight">
              ...
              <span className="text-sm text-gray-700">
                {service.base_price} {t('common.currency', 'ر.ق')}
              </span>
            </div>
            ...
            <button
              onClick={() => {
                ...
                if (navigator.share) {
                  ...
                  toastService.success(
                    t('common.link_copied', 'تم نسخ الرابط'),
                  );
                }
              }}
        ...
        {service.providers && (
          <section className="px-5 py-5 bg-white mb-2">
            <h3 className="text-lg font-bold text-gray-900 mb-3">
              {t('service.details.about_provider', 'عن مقدم الخدمة')}
            </h3>
            <div className="flex items-center gap-4 p-4 bg-bglight rounded-2xl">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <p className="font-bold text-gray-900 truncate">
                    {service.providers.company_name ||
                      t('service.details.provider', 'مقدم الخدمة')}
                  </p>
                ...
                <div className="flex items-center gap-3 text-xs text-gray-500">
                  {service.providers.rating_avg !== undefined &&
                    service.providers.rating_avg > 0 && (
                      <span className="flex items-center gap-1">
                        <i className="fa-solid fa-star text-accent"></i>
                        <span className="font-bold text-gray-700">
                          {Number(service.providers.rating_avg).toFixed(1)}
                        </span>
                      </span>
                    )}
                  ...
                  {service.providers.review_count !== undefined &&
                    service.providers.review_count > 0 && (
                      <span>
                        {service.providers.review_count}{' '}
                        {t('service.details.reviews', 'تقييمات')}
                      </span>
                    )}
                </div>
              </div>
          ...
          return (
            <section className="px-5 py-5 bg-white mb-2">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="bg-orange-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-xs text-gray-500">
                    {t('service.details.notice_days', 'أيام إشعار مسبق')}
                  </p>
                </div>
                <div className="bg-green-50 rounded-xl p-3 text-center">
                  ...
                  <p className="text-xs text-gray-500">
                    {t('service.details.refund_percentage', 'نسبة الاسترداد')}
                  </p>
                </div>
              </div>
              <p className="text-xs text-gray-500 leading-relaxed">
                {description ||
                  t(
                    'service.details.default_cancellation_notice',
                    'يمكن إلغاء الحجز قبل {{days}} أيام من الموعد مع استرداد {{pct}}% من المبلغ.',
                    { days: noticeDays, pct: refundPct },
                  )}
              </p>
            </section>
          );
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-sm font-bold text-gray-700 mb-1.5">
                {t('bookings.event_date', 'تاريخ الفعالية')} *
              </label>
              <input
                type="date"
                value={bookingDate}
                ...
                onChange={(e) => {
                  ...
                  if (unavailableDates.includes(val)) {
                    setDateWarning(
                      t(
                        'bookings.date_unavailable',
                        'هذا التاريخ غير متاح — المزود محجوز بالكامل',
                      ),
                    );
                  } else if (partialDates.includes(val)) {
                    setDateWarning(
                      t(
                        'bookings.date_partial',
                        'هذا التاريخ محجوز جزئياً — تأكد من توافق الأوقات',
                      ),
                    );
                  } else {
                    setDateWarning('');
              ...
              {dateWarning && (
                <p
                  className={`text-xs mt-1 ${unavailableDates.includes(bookingDate) ? 'text-red-500' : 'text-amber-500'}`}
                >
            ...
            <div className="bg-bglight rounded-2xl p-4 flex items-center justify-between">
              <div>
                <p className="text-xs text-gray-500">
                  {t('service.details.base_price', 'السعر الأساسي')}
                </p>
              ...
              <button
                ...
                disabled={
                  bookingLoading || unavailableDates.includes(bookingDate)
                }
              ...
              <button
                onClick={() => {
                  const providerUserId =
                    service.providers?.user_id || service.provider_id;
                  void navigate(
                    `/client/messages?providerId=${providerUserId}&autoStart=true`,
                  );
                }}
        ...
        <section className="px-5 py-5 bg-white mb-2">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-900">
              {t('service.details.reviews', 'التقييمات')} ({reviews.length})
            </h3>
            <div className="flex items-center gap-1">
              <i className="fa-solid fa-star text-accent"></i>
          ...
          {reviews.length === 0 ? (
            <p className="text-center text-gray-400 py-4 text-sm">
              {t('service.details.no_reviews', 'لا توجد تقييمات')}
            </p>
          ) : (
            <div className="mb-4">
              <div className="space-y-3">
                {reviews.slice(0, visibleReviews).map((rev) => (
                  <div
                    key={rev.id}
                    className="bg-bglight/50 rounded-2xl p-4 border border-gray-100"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-gray-900 text-sm">
                          {rev.user_profiles?.full_name ||
                            t('common.user', 'مستخدم')}
                        </span>
                      </div>
              ...
              {reviews.length > 3 && (
                <button
                  onClick={() =>
                    setVisibleReviews((v) => (v >= reviews.length ? 3 : v + 3))
                  }
                ...
                >
                  {visibleReviews >= reviews.length ? (
                    ...
                    <>
                      ...
                      {t('service.listing.load_more', 'تحميل المزيد')} (
          ...
          {isAuthenticated && (
            <div className="border-t border-gray-100 pt-4">
              <h4 className="font-bold text-gray-900 text-sm mb-3">
                {t('service.details.add_review', 'أضف تقييمك')}
              </h4>
              ...
              <textarea
                ...
                placeholder={t(
                  'service.details.review_placeholder',
                  'اكتب تعليقك...',
                )}
              ...
              >
                {submitting
                  ...
                  : t('service.details.submit_review', 'إرسال التقييم')}
              </button>
            </div>
          )}
        </section>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 z-50 flex items-center gap-3 shadow-[0_-5px_20px_rgba(0,0,0,0.05)] lg:hidden">
        <div className="flex-1">
          <p className="text-xs text-gray-600">
            {t('service.details.price_from', 'يبدأ من')}
          </p>
          <p className="text-xl font-bold text-primary">
            {service.base_price?.toLocaleString()} {t('common.currency', 'QAR')}
          </p>
        </div>
        <button
        ...
        <button
          onClick={() => void handleCreateBooking(true)}
