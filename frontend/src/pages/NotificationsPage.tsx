...
const NotificationsPage = () => {
  ...
  const [filter, setFilter] = useState<
    'all' | 'unread' | 'booking' | 'message' | 'quote' | 'payment'
  >('all');
  ...
  useEffect(() => {
    document.title = `${t('common.notifications_label', 'الإشعارات')} | Doha Events`;
  }, [t]);
  ...
  const loadNotifications = useCallback(() => {
    ...
    notificationsService
      .getNotifications(p)
      .then(
        (data: Notification[] | { data?: Notification[]; total?: number }) => {
          const list = Array.isArray(data) ? data : data?.data || [];
          setNotifications(list);
          setTotal(
            !Array.isArray(data) ? (data?.total ?? list.length) : list.length,
          );
        },
      )
      .catch(() =>
        toastService.error(
          t('notifications.error_loading') || 'فشل تحميل الإشعارات',
        ),
      )
  ...
  const markAsRead = async (id: string) => {
    try {
      ...
      toastService.error(
        t('notifications.error_mark_read', 'فشل تحديث الإشعار'),
      );
    }
  };
  ...
  const markAllAsRead = async () => {
    ...
    try {
      ...
      toastService.success(
        t('notifications.mark_all_success') || 'تم تحديد الكل كمقروء',
      );
    } catch (_error) {
      toastService.error(
        t('notifications.mark_all_error') || 'فشل تحديث الإشعارات',
      );
    } finally {
      setMarkingAll(false);
    }
  ...
  const deleteAllRead = async () => {
    const ok = await confirmDialog({
      title: t(
        'notifications.confirm_delete_all_title',
        'Delete Read Notifications',
      ),
      message: t(
        'notifications.confirm_delete_all',
    ...
    try {
      ...
      toastService.success(
        t('notifications.delete_success') || 'تم حذف الإشعارات المقروءة',
      );
    } catch (_error) {
      toastService.error(
        t('notifications.delete_error') || 'فشل حذف الإشعارات',
      );
    }
  };
  ...
  const filtered = notifications.filter((n) => {
    ...
    if (filter === 'payment') return n.type.startsWith('payment');
    return true;
  });
  ...
  const handleNotificationClick = (n: Notification) => {
    ...
    if (n.data?.booking_id)
      navigate(`${rolePrefix}/bookings/${n.data.booking_id}`);
    else if (n.type === 'message') navigate(`${rolePrefix}/messages`);
    else if (n.type.startsWith('quote')) navigate(`${rolePrefix}/quotes`);
    else if (n.type === 'review') navigate(`${rolePrefix}/bookings`);
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">
                {t('common.notifications_label', 'الإشعارات')}
              </h1>
              {unreadCount > 0 && (
      ...
      <div className="px-5 py-3 flex gap-2 overflow-x-auto scrollbar-none">
        {(
          [
            ...
            {
              key: 'booking',
              label: t('notifications.filters.booking'),
              count: notifications.filter((n) => n.type.startsWith('booking'))
                .length,
            },
            {
              key: 'message',
              label: t('notifications.filters.message'),
              count: notifications.filter((n) => n.type === 'message').length,
            },
            {
              key: 'quote',
              label: t('notifications.filters.quote', 'Devis'),
              count: notifications.filter((n) => n.type.startsWith('quote'))
                .length,
            },
            {
              key: 'payment',
              label: t('notifications.filters.payment', 'Paiements'),
              count: notifications.filter((n) => n.type.startsWith('payment'))
                .length,
            },
          ] as const
        ).map((f) => (
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-bold whitespace-nowrap flex items-center gap-1.5 transition-all flex-shrink-0 ${
              filter === f.key
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="bg-white rounded-2xl p-4 shadow-sm animate-pulse flex gap-3"
              >
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-gray-400 text-sm">
              {t('notifications.no_category_match')}
            </p>
          </div>
        ) : (
          (() => {
            const dateLabel = (date: string) => {
              ...
              return d.toLocaleDateString(t('common.date_locale'), {
                weekday: 'long',
                month: 'short',
                day: 'numeric',
              });
                      ...
                      {group.items.map((n) => {
                        ...
                        return (
                          <div
                            ...
                            className={`w-full text-right p-4 rounded-2xl shadow-sm flex items-start gap-3 transition-all hover:shadow-md cursor-pointer ${
                              n.is_read
                                ? 'bg-white'
                          ...
                          >
                            ...
                            >
                              <i
                                className={`fa-solid ${cfg.icon} ${cfg.color}`}
                              ></i>
                            </div>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-start justify-between gap-2">
                                <p
                                  className={`text-sm font-bold truncate ${n.is_read ? 'text-gray-700' : 'text-gray-900'}`}
                                >
                                  {n.title}
                                </p>
