...
const BookingsPage = () => {
  ...
  const fetchBookings = async (status?: string) => {
    ...
    try {
      ...
      const res = (await bookingsService.getMyBookings(`?${p.toString()}`)) as
        | Booking[]
    ...
    <div className="min-h-screen bg-bglight font-tajawal pb-24">
      <header className="bg-white sticky top-0 z-50 shadow-sm px-5 py-4 lg:hidden">
        ...
        <div className="flex gap-2">
          ...
          >
            <i
              className={`fa-solid fa-sort-${sortOrder === 'desc' ? 'down' : 'up'} text-xs`}
            ></i>
            {sortOrder === 'desc'
              ? t('service.listing.sort.newest')
              : t('bookings.sort.oldest')}
          </button>
        </div>
      </header>
      ...
      <main className="px-5 py-2">
        {loading ? (
          ...
          <EmptyState
            ...
            action={{
              label: t('bookings.explore_services'),
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <div>
                            ...
                            <p
                              className={`${booking.services?.title ? 'text-xs text-gray-400' : 'font-bold text-gray-900 text-sm'}`}
                            >
                              {new Date(
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-100">
                      <div>
                        ...
                        <p className="text-lg font-bold text-primary">
                          {booking.amount.toLocaleString()} {t('common.currency', 'QAR')}
                        </p>
                      </div>
                      <div className="text-left">
                        <p className="text-xs text-gray-500">
                          {t('bookings.payment_status')}
                        </p>
                        <p
                          className={`text-sm font-bold ${booking.payment_status === 'fully_paid' ? 'text-green-600' : booking.payment_status === 'deposit_paid' ? 'text-blue-600' : 'text-yellow-600'}`}
                        >
                          {booking.payment_status === 'fully_paid'
                            ? t('bookings.paid')
                            : booking.payment_status === 'deposit_paid'
                              ? t('bookings.partially_paid')
                              : isPartiallyPaid
                                ? t('bookings.partially_paid')
                                : t('bookings.unpaid')}
                        </p>
                        {(booking.payment_status === 'pending' ||
                          ...
                          balanceAmount > 0 && (
                            <p className="text-[11px] text-gray-500">
                              ...
                              {balanceAmount.toLocaleString()} {t('common.currency', 'QAR')}
                            </p>
                          )}
                      </div>
                    </div>
