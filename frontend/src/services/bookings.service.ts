...
export const bookingsService = {
  ...
  getMyBookings: (params = '') =>
    apiService.get<Booking[]>(`/bookings/my-bookings${params}`),
  getByProvider: (providerId: string, params = '') =>
    apiService.get<Booking[]>(`/bookings/provider/${providerId}${params}`),
  ...
  downloadInvoice: async (id: string) => {
    const response = await fetch(`${API_BASE_URL}/payments/invoice/${id}`, {
      credentials: 'include',
      headers: {
