...
const BookingDetailsPage = () => {
  ...
  const handleOpenDispute = async () => {
    ...
    try {
      ...
      toastService.success(
        t('disputes.created', 'تم فتح النزاع — سيراجعه فريق الإدارة'),
      );
    } catch (_error) {
  ...
  useEffect(() => {
    ...
    bookingsService
      ...
      .catch(() =>
        toastService.error(
          t('bookings.error_loading', 'Failed to load booking details'),
        ),
      )
  ...
  const handleCancel = async () => {
    ...
    setCancelling(true);
    try {
      await bookingsService.updateStatus(id, {
        status: 'cancelled',
        cancellation_reason: cancelReason,
      });
      setBooking((prev) =>
        prev
          ? { ...prev, status: 'cancelled', cancellation_reason: cancelReason }
          : prev,
      );
      ...
      toastService.error(
        t('bookings.cancel_error', 'Failed to cancel booking'),
      );
    } finally {
      setCancelling(false);
    }
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      const blob = await bookingsService.downloadInvoice(id);
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `invoice-${id.substring(0, 8).toUpperCase()}.pdf`;
      ...
      toastService.error(
        t('bookings.invoice_download_failed', 'Failed to download invoice'),
      );
    } finally {
    ...
    return (
      <div className="min-h-screen bg-bglight font-tajawal flex flex-col items-center justify-center">
        ...
        <p className="text-gray-500 mb-4">
          {t('bookings.not_found', 'Booking not found')}
        </p>
        <button
          onClick={() => navigate(-1)}
  ...
  const paymentStatusLabel = (status?: string, partial?: boolean) => {
    ...
    if (status === 'deposit_paid')
      return t('status.deposit_paid', 'Deposit paid');
  ...
  const st = statusMap[booking.status] || {
  ...
  const canPay =
    ...
    (booking.payment_status === 'pending' ||
      booking.payment_status === 'deposit_paid');
  const canDownloadInvoice = ['deposit_paid', 'fully_paid'].includes(
    booking.payment_status || '',
  );
  const hasPaymentDocument = booking.receipt_url || canDownloadInvoice;
        ...
        {hasPaymentDocument && (
          <div className="bg-white rounded-2xl p-5 shadow-sm">
            <h3 className="font-bold text-gray-900 mb-3 flex items-center gap-2">
              ...
              {t('bookings.payment_documents', 'Payment documents')}
            </h3>
            <div className="flex flex-wrap gap-2">
              {canDownloadInvoice && (
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 text-primary font-bold hover:bg-primary/20 transition-colors disabled:opacity-60"
                >
                  {downloadingInvoice ? (
                  ...
                  {t('bookings.download_invoice', 'Download invoice')}
                </button>
              )}
              {booking.receipt_url && (
                <a
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-gray-100 text-gray-700 font-bold hover:bg-gray-200 transition-colors"
                >
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-sm text-gray-900">
                {new Date(booking.booking_date).toLocaleDateString(locale, {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                })}
            ...
            {booking.locked_price && booking.locked_price !== totalAmount && (
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm text-gray-400 line-through">
                  {booking.locked_price.toLocaleString(locale)} {currency}
                </span>
              </div>
            )}
            ...
            {typeof booking.platform_fee === 'number' &&
              booking.platform_fee > 0 && (
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-sm text-gray-900">
                    {booking.platform_fee.toLocaleString(locale)} {currency}
                  </span>
                </div>
              )}
            ...
            {booking.deposit_amount ? (
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-sm text-gray-900">
                  {Number(booking.deposit_amount).toLocaleString(locale)}{' '}
                  {currency}
                </span>
              </div>
            ) : null}
            ...
            {payableAmount > 0 && (
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-sm text-orange-600">
                  {payableAmount.toLocaleString(locale)} {currency}
                </span>
              </div>
            )}
        ...
        <div className="space-y-3 pt-2">
          {canPay && (
            <button
              onClick={() =>
                navigate(
                  `/client/payment/${booking.id}?amount=${payableAmount}&paymentType=${paymentType}&service=${encodeURIComponent(booking.services?.title || '')}`,
                )
              }
            ...
            >
              ...
              {paymentType === 'balance'
                ? t('payments.pay_remaining', 'Pay remaining')
                : t('payments.pay_now', 'Pay now')}{' '}
              - {payableAmount.toLocaleString(locale)} {currency}
            </button>
          )}
          ...
          <button
            onClick={() =>
              navigate(
                `/client/messages?providerId=${booking.provider_id}&autoStart=true`,
              )
            }
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md p-6 shadow-2xl">
            <h3 className="font-bold text-lg text-gray-900 mb-2">
              {t('bookings.cancel_modal_title', 'Cancel booking')}
            </h3>
            <p className="text-sm text-gray-500 mb-4">
              {t(
                'bookings.cancel_modal_body',
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-700 mb-1.5">
                {t(
                  'bookings.cancel_reason_optional',
            ...
            <div className="flex gap-3">
              ...
              >
                {cancelling
                  ...
                  : t('bookings.confirm_cancel', 'Confirm cancellation')}
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-black/50 flex items-end md:items-center justify-center z-50 p-4">
          <div className="bg-white rounded-t-3xl md:rounded-3xl w-full max-w-md p-6 shadow-2xl">
            ...
            <p className="text-sm text-gray-500 mb-4">
              {t(
                'disputes.open_desc',
                'صف المشكلة وسيراجعها فريق الإدارة ويتخذ القرار المناسب.',
              )}
            </p>

            <div className="mb-3">
              <label className="block text-sm font-bold text-gray-700 mb-1.5">
                {t('disputes.reason', 'سبب النزاع')} *
              </label>
              <select
                value={disputeReason}
              ...
              >
                <option value="service_not_delivered">
                  {t('disputes.reasons.not_delivered', 'لم يتم تقديم الخدمة')}
                </option>
                <option value="quality_issue">
                  {t('disputes.reasons.quality', 'مشكلة في الجودة')}
                </option>
                <option value="late_arrival">
                  {t('disputes.reasons.late', 'تأخر في الوصول')}
                </option>
                <option value="wrong_service">
                  {t('disputes.reasons.wrong', 'خدمة مختلفة عن المتفق عليها')}
                </option>
                <option value="overcharged">
                  {t('disputes.reasons.overcharged', 'مبلغ زائد')}
                </option>
                <option value="provider_no_show">
                  {t('disputes.reasons.no_show', 'المزود لم يحضر')}
                </option>
                <option value="other">
                  {t('disputes.reasons.other', 'سبب آخر')}
                </option>
              </select>
            </div>
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-700 mb-1.5">
                {t('disputes.description', 'وصف المشكلة')} *
              </label>
              <textarea
                value={disputeDesc}
