...
import {
  ...
  getBodyText, elementExists,
} from './hooks';

// ── Quotes & Messaging Steps ────────────────────

When('je remplis le message {string}', async function (msg: string) {
  if (await elementExists(this, 'textarea')) { await typeInto(this, 'textarea', msg); }
});

When('je spécifie la date souhaitée {string}', async function (d: string) {
  if (await elementExists(this, 'input[type="date"]')) { await typeInto(this, 'input[type="date"]', d); }
});
