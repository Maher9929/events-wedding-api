        ...
        <div className={`rounded-xl overflow-hidden border ${isSender ? 'border-purple-300 bg-purple-50' : 'border-gray-200 bg-white'} min-w-[300px]`}>
            ...
            <div className={`p-3 flex items-center justify-between ${isSender ? 'bg-purple-100' : 'bg-gray-100'}`}>
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-sm text-gray-900">عرض سعر</span>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-md font-bold ${status === 'accepted' ? 'bg-green-100 text-green-700' :
                        status === 'rejected' ? 'bg-red-100 text-red-700' :
                            'bg-yellow-100 text-yellow-700'
                    }`}>
                    {status === 'sent' ? 'بانتظار الرد' : status}
                </span>
            </div>
                ...
                <ul className="space-y-2 mb-3">
                    {items.map((item, idx) => (
                        <li key={idx} className="flex justify-between text-xs text-gray-600">
                            ...
                            <span className="font-mono">{(item.price * item.quantity).toLocaleString()} ر.ق</span>
                        </li>
                    ))}
                </ul>

                <div className="border-t border-gray-200 pt-2 flex justify-between items-center mb-3">
                    <span className="font-bold text-sm text-gray-900">المجموع</span>
                    <span className="font-bold text-lg text-primary">{total.toLocaleString()} ر.ق</span>
                </div>

                {/* Actions - Only show for receiver if pending */}
